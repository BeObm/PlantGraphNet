...
def baseline_model(model_name, num_classes):
    if model_name == "AlexNet":
        return_ = AlexNet_Model(num_classes)
    elif model_name == "MobileNetV2":
        return_ = MobileNetV2_Model(num_classes)
    elif model_name == "ResNet50":
        return_ = ResNet50_Model(num_classes)
    elif model_name == "ResNet101":
        return_ = ResNet101_Model(num_classes)
    elif model_name == "VGG16":
        return_ = VGG16_Model(num_classes)
    elif model_name == "VGG19":
        return_ = VGG19_Model(num_classes)
    elif model_name == "GoogleNet":
        return_ = GoogleNet_Model(num_classes)
    elif model_name == "Unet":
        return_ = UNetClassifier(num_classes=num_classes)
...
class YOLOv8Classifier(nn.Module):
    def __init__(self, pretrained_model='yolov8m-cls.pt', num_classes=10):
        super().__init__()
        self.yolo = YOLO(pretrained_model)

        # Extract backbone layers except the last layer
        self.backbone = nn.Sequential(*list(self.yolo.model.model)[:-1])
...
class YOLOv11xClassifier(YOLOv8Classifier):
    def __init__(self, num_classes=10):
